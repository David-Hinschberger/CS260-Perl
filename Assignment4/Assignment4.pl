...
my $fileName = ($ARGV[0] && ($foo = 1) && print "Input file: $ARGV[0]\n") ? $ARGV[0] : (print "Input file: ") ? inputchomp : 0;
print "Could not read file: $fileName" and exit unless (defined open(INPUT, $fileName));
((push @data, $_) && ($foo-- > 0 && (@legend = qw([0] [1] [2ABC] [3DEF] [4GHI] [5JKL] [6MNO] [7PQRS] [8TUV] [9WXYZ])))) while(chomp($_ = <INPUT>) >= 0 && $_);
while(((print"Search: ") && (($searchstr = '') != ($fileName = -1)))) {
    $searchstr .= (($foo = $_) && (@_ = grep(/$foo/i, @legend))[0] . '+.*') foreach (((@search = split (undef , inputchomp )) and (grep(/[^A-Z0-9]/i, @search)))? last : ((scalar @search == 0))? last : @search);
